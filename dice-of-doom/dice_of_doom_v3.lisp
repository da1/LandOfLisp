;; 19章ダイスオブドゥームにグラフィカルなWebインタフェースをつける
           ...
           (tag center ()
                ...
                (let ((chosen (assoc 'chosen params)))
                  (when (or (not *cur-game-tree*) (not chosen))
                    (setf chosen nil)
                    (web-initialize))
                  (cond ((lazy-null (available-action *cur-game-tree*))
                         (web-announce-winner (current-board *cur-game-tree*)))
...
(defun web-handle-human (pos)
  (cond ((not pos) (princ "Please choose a hex to move from:"))
        ((eq pos 'pass) (setf *cur-game-tree*
                              (cadr (lazy-car (available-action *cur-game-tree*))))
...
(defun draw-dod-page (tree selected-tile)
  (svg *board-width*
       *board-height*
       (draw-board-svg (current-board tree)
                       selected-tile
                       (take-all (if selected-tile
                                   ...
                                   (lazy-mapcar #'caar (available-action tree)))))))
